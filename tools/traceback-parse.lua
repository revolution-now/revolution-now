...
remove_lines_containing = {
  '======', 'single_include', 'compiler.rt',
  'is located.*bytes inside', 'libc_start_main',
  'unique.func.class', 'unique.func.hpp',
}

keep_lines_identical_containing = { '^$', 'heap.use.after.free' }
...
function freeze_table( tbl )
  setmetatable( tbl, {
    __newindex=function( _, k, v )
      error( 'attempt to add key "' .. tostring( k ) ..
                 '" to frozen table.', 2 )
    end,
  } )
end
...
for line in io.lines() do
  ...
  for _, v in pairs( remove_lines_containing ) do
    if line:find( v ) then goto loop_end end
  end
