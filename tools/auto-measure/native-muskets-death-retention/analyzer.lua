...
local function probability_saves_muskets_table(main_config,
                                               global_stats )
  ...
  return table_def( main_config, global_stats,
                    'Probability Saves Muskets', calculator )
end
...
function M.post_config_setup( _, json )
  ...
  -- Check that we have a single armed brave.
  local brave_count = 0
  local armed_brave_count = 0
  on_all_braves( json, function( brave )
    brave_count = brave_count + 1
    if brave.type == 'Armed brave' then
      armed_brave_count = armed_brave_count + 1
    end
  end )
  assert( brave_count == 1 )
  assert( armed_brave_count == 1 )

  on_all_tribes( json, function( _, tribe ) tribe.muskets = 0 end )
...
function M.action( api )
  local right = assert( api.right )

  -- This action is to be run when there is an armed brave to the
  -- right of a european military unit that is waiting for or-
  -- ders.

  -- Make the european unit attack the armed brave.
  sleep( 1.0 )
  right()
  sleep( 2.5 ) -- Attack animation.
  -- A message sometimes pops up giving results. Use the right
  -- key to close it, that way if there is no message it won't
  -- cause any issues, it'll just move the white selector square
  -- to the right, since presumably we've enabled end-of-turn.
  right()
...
function M.summary_tables( main_config, global_stats )
  return {
    probability_saves_muskets_table( main_config, global_stats ),
  }
end
