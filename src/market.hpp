...
// Note that this will not actually make the purchase, it just
// describes the results (not including any subsequent price
// changes that might happen were it to occur).
...
// not check or care.
void buy_commodity( SS& ss, Player& player,
                    PurchaseInvoice const& invoice );

// Similar to the above but for sales.
void sell_commodity( SS& ss, Player& player,
                     SaleInvoice const& invoice );
...
PriceChange compute_price_change(
    Player const& player,
