...
double SmoothViewport::width_tiles() const {
  int lower =
      round_down_to_nearest_int_multiple( start_x(), 32 );
  int upper = round_up_to_nearest_int_multiple( end_x(), 32 );
  return upper - lower;
}
...
double SmoothViewport::height_tiles() const {
  int lower =
      round_down_to_nearest_int_multiple( start_y(), 32 );
  int upper = round_up_to_nearest_int_multiple( end_y(), 32 );
  return upper - lower;
}
...
void SmoothViewport::enforce_invariants() {
  ...
  if( start_x() < 0 ) center_x_ = width_pixels() / 2.0;
  if( start_y() < 0 ) center_y_ = height_pixels() / 2.0;
  if( end_x() > size_x )
    center_x_ =
        double( size_x ) - double( width_pixels() ) / 2.0;
  if( end_y() > size_y )
    center_y_ =
        double( size_y ) - double( height_pixels() ) / 2.0;
}
...
Rect SmoothViewport::get_render_dest_rect() const {
  ...
  if( viewport.w > max_src_width ) {
    double delta = ( double( viewport.w - max_src_width ) /
                     double( viewport.w ) ) *
                   g_tile_width._ *
                   double( int( viewport_width_tiles() ) ) / 2.0;
    dest.x += int( delta );
    dest.w -= int( delta * 2 );
  }
  if( viewport.h > max_src_height ) {
    double delta = ( double( viewport.h - max_src_height ) /
                     double( viewport.h ) ) *
                   g_tile_height._ *
                   double( int( viewport_height_tiles() ) ) /
                   2.0;
    dest.y += int( delta );
    dest.h -= int( delta * 2 );
  }
...
bool is_tile_surroundings_fully_visible(
    SmoothViewport const& vp, Coord coords ) {
