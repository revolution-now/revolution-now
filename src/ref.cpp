...
maybe<RefLanding> produce_REF_landing_units(
  ...
  if( !metrics.has_value() ) {
    ...
    // detected later in the REF's turn. The latter can happen
    // in the following scenarios:
    //
    //   1. Colony on an island, including a long arctic tile.
    //      The former don't get generated by the standard map
    //      but the latter do. And in any case the player could
    //      be using a custom map.
    //   2. Colony whose only surrounding land tiles are occupied
    //      by native dwellings. This can happen in the standard
    //      map generator and the OG allows them as well, though
    //      they are very uncommon.
    //
    // The first scenario (island) is already handled in that the
    // game by default prevents founding on islands, and if the
    // player enables it they will at least be given a warning
