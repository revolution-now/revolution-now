...
** RiverStatsCollector
*****************************************************************/
struct RiverStatsCollector : IMapStatsCollector {
  RiverStatsCollector( string const& mode,
  ...
  on_all_tiles(
      m, [&]( point const tile, MapSquare const& center ) {
        ...
        wetness_on_biome_by_col_[tile.x][center.ground] +=
            wetness.m[tile];
        land_wetness_ += wetness.m[tile];
        wetness_by_row_[tile.y] += wetness.m[tile];
        wetness_by_col_[tile.x] += wetness.m[tile];
      } );
}
...
unique_ptr<IMapStatsCollector> create_river_stats_collector(
    string const& stem, double const tolerance ) {
  return make_unique<RiverStatsCollector>( stem, tolerance );
}
