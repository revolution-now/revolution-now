...
ADT_RN(
    ...
    // they are on the dock. Each time a unit is placed in port
    // or on the dock a new id is generated by incrementing the
    // previously used one, and this allows the units in port to
    // be sorted by their arrival time, otherwise it might be vi-
    // sually confusing to the player since e.g. the order of
    // units on the dock would not be deterministic.
    ( in_port,                     //
      ( int, global_arrival_id ) ) //
);
