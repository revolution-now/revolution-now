...
local function create_some_units_on_land( nation1, nation2 )
  ...
  local unit = ustate.create_unit_on_map(
           nation1, e.unit_type.free_colonist, coord )
  unit:fortify();

  coord = Coord{y=6, x=3}
  unit = ustate.create_unit_on_map(
           nation1, e.unit_type.soldier, coord )
  unit:sentry();

  coord = Coord{y=6, x=6}
  ustate.create_unit_on_map(
      nation1, e.unit_type.privateer, coord )

  coord = Coord{y=7, x=2}
  unit = ustate.create_unit_on_map(
           nation2, e.unit_type.soldier, coord )
  unit:fortify();
  ...
  coord = Coord{y=7, x=6}
  unit = ustate.create_unit_on_map(
           nation2, e.unit_type.privateer, coord )
  unit:clear_orders();
end
...
local function create_some_colonies( nation1, nation2 )
  ...
  coord = Coord{y=4, x=4}
  unit = ustate.create_unit_on_map(
           nation2, e.unit_type.free_colonist, coord )
