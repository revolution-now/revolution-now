...
function M.initial_ships_pos()
  ...
  local s1 = quintile
  local s2 = max( quintile * 2, s1 + 1 )
  local s3 = max( quintile * 3, s2 + 1 )
  local s4 = max( quintile * 4, s3 + 1 )
  local quintiles = { s1, s2, s3, s4 }
  shuffle( quintiles )
...
function M.generate_land( options )
  ...
  if options.remove_crosses then map_gen.remove_crosses() end
