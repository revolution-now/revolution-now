...
int unit_sight_radius( SSConst const& ss,
  ...
  if( has_de_soto ) {
    if( !unit_attr( type ).ship )
      ...
      // The original game states that De Soto will give all
      // units an extended sighting radius. The meaning of this
      // apparently is that it takes the sighting radius of each
      // unit and adds one to it (even if it was already 2),
      // since that is what it does for land units. However, it
      // does not do this for ships at all. In this game we de-
      // fault to allowing the bonus to apply to ships as well in
