...
TEST_CASE( "CargoHold add/remove from size-1 cargo hold" ) {
  ...
  auto cargo = GENERATE(
      CargoSlot::cargo{ /*contents=*/create_unit(
          e_nation::english, e_unit_type::free_colonist ) },
  ...
  SECTION( "specify slot" ) {
    ...
    if( auto* comm = get_if<Commodity>( &( cargo.contents ) ) ) {
      REQUIRE_THAT(
          ch.commodities(),
          UnorderedEquals(
              vector<pair<Commodity, int>>{ { *comm, 0 } } ) );
      REQUIRE( ch.debug_string() ==
  ...
  SECTION( "specify middle slot" ) {
    ...
    if( auto* comm = get_if<Commodity>( &( cargo.contents ) ) ) {
      REQUIRE_THAT(
          ch.commodities(),
          UnorderedEquals(
              vector<pair<Commodity, int>>{ { *comm, 3 } } ) );
      REQUIRE( ch.debug_string() ==
...
TEST_CASE( "CargoHold try add as available from all (units)" ) {
  ...
  for( auto i = 0; i < 6; ++i ) {
    ...
    cmp_slots[( i + start ) % 6] = CargoSlot_t{
        CargoSlot::cargo{ /*contents=*/unit_ids[i] } };
    REQUIRE( ch.slots_ == cmp_slots );
  }
...
TEST_CASE(
  ...
  SECTION( "unlike types" ) {
    ...
    REQUIRE(
        ...
        CargoSlot_t{ CargoSlot::cargo{ /*contents=*/Commodity{
            /*type=*/e_commodity::food, /*quantity=*/100 } } } );
    REQUIRE( ch[next] == CargoSlot_t{ CargoSlot::cargo{
                             /*contents=*/Commodity{
                                 /*type=*/e_commodity::sugar,
                                 /*quantity=*/100 } } } );
    REQUIRE(
        ch[next2] ==
        CargoSlot_t{ CargoSlot::cargo{ /*contents=*/Commodity{
...
TEST_CASE( "CargoHold fits_somewhere_with_item_removed" ) {
  ...
  SECTION( "insert large unit" ) {
    ...
    REQUIRE( ch.fits_somewhere_with_item_removed(
        unit_id3, /*remove_slot=*/1, /*starting_slot=*/0 ) );

    REQUIRE( ch.try_add( unit_id4, 5 ) );
  ...
  SECTION( "insert samll units" ) {
    REQUIRE( ch.try_add( unit_id1, 1 ) );
    REQUIRE( ch.fits_somewhere_with_item_removed(
        unit_id2, /*remove_slot=*/1, /*starting_slot=*/0 ) );
    REQUIRE( ch.fits_somewhere_with_item_removed(
        unit_id2, /*remove_slot=*/1, /*starting_slot=*/1 ) );

    REQUIRE( ch.try_add( unit_id4, 4 ) );
    REQUIRE( ch.fits_somewhere_with_item_removed(
        unit_id2, /*remove_slot=*/4, /*starting_slot=*/0 ) );
    REQUIRE( ch.fits_somewhere_with_item_removed(
        unit_id2, /*remove_slot=*/4, /*starting_slot=*/1 ) );

    REQUIRE( ch.try_add( unit_id5, 3 ) );
    REQUIRE_FALSE( ch.fits_somewhere_with_item_removed(
